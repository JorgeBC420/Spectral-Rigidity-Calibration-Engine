...
if __name__ == "__main__":
    ...
    if RIGIDEZ_DISPONIBLE:
        ...
        print(f"Ejecutando protocolo de escalamiento para N = {N_RIGIDEZ}...")
        print(f"Esto incluye: Riemann, Uniforme, GUE (5 realizaciones), Poisson\n")
        
        try:
            datos_rigidez = ejecutar_analisis_completo(CACHE.obtener, N_values=N_RIGIDEZ)
            print("\n? Protocolo de rigidez espectral completado")
        except Exception as e:
            print(f"??  Error durante protocolo de rigidez: {e}")
            import traceback
            traceback.print_exc()
    else:
